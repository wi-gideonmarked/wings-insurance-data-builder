...
async function fetchModelsForMake(client, makeId) {
  // Get models with their provider mappings
  const sql = `
    SELECT 
      m.id,
      m.hash,
...
async function main() {
  ...
  if (!process.env.PGHOST || !process.env.PGUSER || !process.env.PGDATABASE) {
    console.log('[export-from-flexible-schema] Skipped: PGHOST/PGUSER/PGDATABASE not set');
    return;
  }
  ...
  try {
    ...
    console.log(`[export-from-flexible-schema] Found ${makes.length} makes`);
    
    for (const make of makes) {
      const items = await fetchModelsForMake(client, make.id);
      const slug = make.slug; // Use the slug from database instead of generating
      const filePath = path.join(destDir, `${slug}.json`);
      fs.writeFileSync(filePath, JSON.stringify(items, null, 2), 'utf8');
      console.log(`[export-from-flexible-schema] Wrote ${items.length.toString().padStart(4)} items -> ${filePath} (${make.name})`);
    }
    
    console.log(`[export-from-flexible-schema] Done. Output directory: ${destDir}`);
  } finally {
...
main().catch((e) => {
  console.error('[export-from-flexible-schema] ERROR:', e?.message || e);
  process.exit(1);
});
